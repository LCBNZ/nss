...
import { Button, FormDrawer, Input, Spinner } from "../../common";

import { FilesApi } from "../../api";
import supabase from "../../api/supabase";

export const CreateFile = ({ column, type, id }) => {
  ...
  const onDrop = useCallback(async (files) => {
    if (files?.length > 0 && files?.length < 2) {
      ...
      if (!error) {
        const fetchFile = await supabase.storage.from("files").getPublicUrl(`${type}/${fileName}`);
  ...
  const removeUploadedFile = async () => {
    ...
    if (fileName) {
      const removeFile = await supabase.storage.from("files").remove([`${type}/${fileName}`]);
  ...
  return (
    <FormDrawer
      isDone={createFileMutation.isSuccess}
      triggerButton={
    ...
    >
      ...
      >
        {({ values, handleChange, handleBlur, handleSubmit, setFieldValue, setFieldTouched }) => (
          <form onSubmit={handleSubmit} id="create-file">
            <div className="">
              <Input
                title="File Type"
                id="file_type"
